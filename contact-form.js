(function () {
  ...
  var MESSAGES = {
    ru: {
      ...
      genericError: 'Ошибка отправки. Попробуйте позже.',
    ...
    ka: {
      nameRequired: 'გთხოვთ, მიუთითოთ, როგორ მოგვიხმოთ.',
      emailInvalid: 'გთხოვთ, შეიყვანოთ სწორი ელფოსტის მისამართი.',
      messageTooShort: 'შეტყობინება ძალიან მოკლეა (მინიმუმ 10 სიმბოლო).',
      messageTooLong: 'შეტყობინება ძალიან გრძელია (მაქსიმუმ 5000 სიმბოლო).',
      fillAllFields: 'შეტყობინების გასაგზავნად შეავსეთ ყველა ველი.',
      notConfigured:
        'ფორმა ჯერ არ არის დაკავშირებული ელფოსტასთან. მიუთითეთ CONTACT_FORM_ENDPOINT და CONTACT_SUBJECT_PREFIX data/contact-config.js-ში.',
      sending: 'იგზავნება…',
      success:
        'გმადლობთ! შეტყობინება ტურის ორგანიზატორებს გაეგზავნა.\nჩვენ გიპასუხებთ მითითებულ ელფოსტაზე.',
      honeypotSuccess: 'გმადლობთ! შეტყობინება გაიგზავნა.',
      serverNoJson:
        'ფორმის სერვერი არ პასუხობს (მოსალოდნელი იყო JSON). შეამოწმეთ Render-ის დეპლოი.',
      sendFailed: 'შეტყობინების გაგზავნა ვერ მოხერხდა.',
      networkError: 'ფორმის სერვერთან დაკავშირება ვერ მოხერხდა. სცადეთ მოგვიანებით.',
      genericError: 'გაგზავნა ვერ მოხერხდა. სცადეთ მოგვიანებით.',
      rateLimit: 'გთხოვთ, ერთი წუთის შემდეგ სცადოთ ხელახლა.',
      nameTooLong: 'სახელი ძალიან გრძელია.',
      siteNotConfigured: 'ფორმა ამ საიტისთვის არ არის კონფიგურირებული. დაუკავშირდით ადმინისტრატორს.',
      noSubjectPrefix: 'subjectPrefix არ არის მითითებული.',
      smtpNotConfigured: 'საფოსტო სერვერი არ არის კონფიგურირებული. სცადეთ მოგვიანებით.',
    },
  };
  ...
  function translateServerError(form, rawMessage) {
    ...
    if (
      ...
      text.indexOf('მოგვიხმოთ') !== -1 ||
      text.indexOf('Имя слишком') !== -1
    ) {
      if (
        text.indexOf('длинн') !== -1 ||
    ...
    if (
      text.indexOf('длинн') !== -1 ||
      text.indexOf('too long') !== -1 ||
      (text.indexOf('გრძელი') !== -1 && text.indexOf('შეტყობინებ') !== -1)
    ) {
      return { message: msg(form, 'messageTooLong'), field: 'message' };
    }

    if (text.indexOf('минут') !== -1 || text.indexOf('minute') !== -1 || text.indexOf('წუთ') !== -1) {
  ...
  async function sendToMailApi(data) {
    var response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        ...
        source: window.location.hostname || 'serebrennikov.ge',
