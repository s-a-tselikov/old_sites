(function () {
  ...
  function injectLayoutStyles() {
    ...
    style.textContent =
      ...
      '#rec556941186 .t698,#rec556941186 form,#rec556941186 .t-form__submit,' +
      '#rec558926321 .t698,#rec558926321 form,#rec558926321 .t-form__submit{pointer-events:auto;}';
  ...
  function showValidationErrors(form, errors) {
    ...
    var message =
      errors.length > 2
        ? uiText('fillAll', 'Для отправки сообщения заполните все поля формы.')
        : errors
            .map(function (err) {
              return err.message;
            })
  ...
  function mapServerError(rawMessage) {
    ...
    if (lower.indexOf('имя') !== -1 || lower.indexOf('обращаться') !== -1) {
      if (lower.indexOf('длинн') !== -1) {
        return {
          message: uiText('nameTooLong', 'Имя слишком длинное.'),
          field: 'name',
        };
      }
      return {
        message: uiText('nameRequired', 'Укажите, как к вам обращаться.'),
        field: 'name',
      };
    }
    ...
    if (lower.indexOf('email') !== -1 || lower.indexOf('e-mail') !== -1) {
      return {
        message: uiText('emailInvalid', 'Укажите корректный email.'),
        field: 'email',
      };
    }
    ...
    if (lower.indexOf('минут') !== -1) {
      return {
        message: uiText('rateLimit', 'Подождите минуту перед повторной отправкой.'),
        field: null,
      };
    }
    ...
    if (lower.indexOf('не настроена для сайта') !== -1) {
      return {
        message: uiText(
          'siteNotAllowed',
          'Форма не настроена для этого сайта. Обратитесь к администратору.'
        ),
        field: null,
      };
    }
    ...
    return {
      message: text || uiText('sendFailed', 'Не удалось отправить сообщение.'),
      field: null,
    };
  }
  ...
  async function onSubmit(event) {
    ...
    try {
      ...
      if (data.website) {
        form.reset();
        setStatus(form, uiText('honeypotSuccess', 'Спасибо! Сообщение отправлено.'), true);
        return;
      }
      ...
      setStatus(form, uiText('sending', 'Отправляется…'));
      ...
      try {
        ...
        var msg = sendError.message || uiText('sendFailed', 'Ошибка отправки. Попробуйте позже.');
        if (msg === 'Failed to fetch') {
          msg = uiText(
            'networkError',
            'Не удалось связаться с сервером формы. Попробуйте позже.'
          );
        }
